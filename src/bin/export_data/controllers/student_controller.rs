...
pub async fn student_export_csv(state: web::Data<AppState>, body: web::Json<GeneralSchema>) -> impl Responder {
  ...
  match handle_student_export(state.db.clone(), body.into_inner(), table.to_owned(), String::from("csv")).await {
...
pub async fn student_export_excel(state: web::Data<AppState>, body: web::Json<GeneralSchema>) -> impl Responder {
  ...
  match handle_student_export(state.db.clone(), body.into_inner(), table.to_owned(), String::from("xlsx")).await {
    Ok((data, mime_type, file_name)) => create_response_binary(
      String::from("success"),
      data,
      format!("attachment; filename={}", file_name),
